...
function deepClone(candidate) {
	...
	if ((typeof candidate === 'undefined' ? 'undefined' : _typeof(candidate)) !== "object" || candidate === null) {
		return candidate;
	}
...
function shortenLargeNumber(label) {
    ...
    var shortened = Math.pow(10, p - l * 3) * +(number / Math.pow(10, p)).toFixed(1);
...
function getSplineCurvePointsStr(xList, yList) {
    ...
    var pointStr = function pointStr(points, command) {
        return points.reduce(function (acc, point, i, a) {
            return i === 0 ? point[0] + ',' + point[1] : acc + ' ' + command(point, i, a);
        }, '');
    };
...
var getColor = function getColor(color) {
	...
	if (/rgb[a]{0,1}\([\d, ]+\)/gim.test(color)) {
		return (/\D+(\d*)\D+(\d*)\D+(\d*)/gim.exec(color).map(function (x, i) {
				return i !== 0 ? Number(x).toString(16) : '#';
...
function generateAxisLabel(options) {
    ...
    var x = options.position === 'left' ? LABEL_MARGIN : options.width;
    // - getStringWidth(options.title, 5);
    var rotation = options.position === 'right' ? 'rotate(90, ' + options.width + ', ' + options.height / 2 + ')' : 'rotate(270, 0, ' + options.height / 2 + ')';
...
function yLine(y, label, width) {
    ...
    var offset = options.pos === 'left' ? -1 * options.offset : options.offset;
...
function animatePath(paths, newXList, newYList, zeroLine, spline) {
	...
	var animPath = [paths.path, { d: "M" + pointsStr }, PATH_ANIM_DUR, STD_EASING];
	...
	if (paths.region) {
		...
		var animRegion = [paths.region, { d: "M" + regStartPt + pointsStr + regEndPt }, PATH_ANIM_DUR, STD_EASING];
	...
	createClass(AggregationChart, [{
		...
		value: function renderLegend() {
			...
			this.legendTotals.map(function (d, i) {
				...
				if (_this3.legendTotals.length < divisor) {
					barWidth = _this3.width / _this3.legendTotals.length;
				}
				if (count > divisor) {
					count = 0;
					y += 20;
				...
				var label = _this3.config.truncateLegends ? truncateString(s.labels[i], barWidth / 10) : s.labels[i];
				var formatted = _this3.config.formatTooltipY ? _this3.config.formatTooltipY(d) : d;
				var dot = legendDot(x, y, 5, _this3.colors[i], label + ': ' + formatted, false);
    ...
    createClass(ChartComponent, [{
        ...
        value: function render(data) {
            ...
            this.store.forEach(function (element) {
                element.length ? element.forEach(function (el) {
                    _this.layer.appendChild(el);
...
var componentConfigs = {
    ...
    yAxis: {
        ...
        makeElements: function makeElements(data) {
            ...
            if (data.length) {
                data.forEach(function (item, i) {
                    item.positions.map(function (position, i) {
                        elements.push(yLine(position, item.labels[i], _this3.constants.width, {
                            mode: _this3.constants.mode,
                            pos: item.pos || _this3.constants.pos,
                            shortenNumbers: _this3.constants.shortenNumbers
                        }));
                    });
                    // we need to make yAxis titles if they are defined
                    if (item.title) {
                        elements.push(generateAxisLabel({
                            title: item.title,
                            position: item.pos,
                            height: item.zeroLine,
                            width: _this3.constants.width
                        }));
                    }
                });
        ...
        animateElements: function animateElements(newData) {
            ...
            var animateMultipleElements = function animateMultipleElements(oldData, newData) {
                var newPos = newData.positions;
                var newLabels = newData.labels;
                var oldPos = oldData.positions;
                var oldLabels = oldData.labels;
                ...
                return _this4.store.map(function (line, i) {
            ...
            if (this.oldData instanceof Array) {
                return this.oldData.forEach(function (old, i) {
    ...
    xAxis: {
        ...
        makeElements: function makeElements(data) {
            ...
            return data.positions.map(function (position, i) {
                return xLine(position, data.calcLabels[i], _this5.constants.height, {
                    mode: _this5.constants.mode,
                    pos: _this5.constants.pos
                });
            });
        },
        animateElements: function animateElements(newData) {
            ...
            var _equilizeNoOfElements9 = equilizeNoOfElements(oldPos, newPos);

            var _equilizeNoOfElements10 = slicedToArray(_equilizeNoOfElements9, 2);

            oldPos = _equilizeNoOfElements10[0];
            newPos = _equilizeNoOfElements10[1];

            var _equilizeNoOfElements11 = equilizeNoOfElements(oldLabels, newLabels);

            var _equilizeNoOfElements12 = slicedToArray(_equilizeNoOfElements11, 2);

            oldLabels = _equilizeNoOfElements12[0];
            newLabels = _equilizeNoOfElements12[1];
    ...
    yMarkers: {
        ...
        makeElements: function makeElements(data) {
            ...
            return data.map(function (m) {
                return yMarker(m.position, m.label, _this6.constants.width, {
        ...
        animateElements: function animateElements(newData) {
            var _equilizeNoOfElements13 = equilizeNoOfElements(this.oldData, newData);

            var _equilizeNoOfElements14 = slicedToArray(_equilizeNoOfElements13, 2);

            this.oldData = _equilizeNoOfElements14[0];
            newData = _equilizeNoOfElements14[1];
    ...
    yRegions: {
        ...
        makeElements: function makeElements(data) {
            ...
            return data.map(function (r) {
                return yRegion(r.startPos, r.endPos, _this7.constants.width, r.label, {
                    labelPos: r.options.labelPos
                });
            });
        },
        animateElements: function animateElements(newData) {
            var _equilizeNoOfElements15 = equilizeNoOfElements(this.oldData, newData);

            var _equilizeNoOfElements16 = slicedToArray(_equilizeNoOfElements15, 2);

            this.oldData = _equilizeNoOfElements16[0];
            newData = _equilizeNoOfElements16[1];
    ...
    heatDomain: {
        ...
        makeElements: function makeElements(data) {
            ...
            data.cols.map(function (week, weekNo) {
                if (weekNo === 1) {
                    _this8.labels.push(makeText('domain-name', x, monthNameHeight, getMonthName(index, true).toUpperCase(), {
                        fontSize: 9
                    }));
                }
                week.map(function (day, i) {
                    if (day.fill) {
                        ...
                        _this8.serializedSubDomains.push(square);
                    }
                    y += rowHeight;
                });
                y = 0;
    ...
    barGraph: {
        ...
        animateElements: function animateElements(newData) {
            ...
            var _equilizeNoOfElements17 = equilizeNoOfElements(oldXPos, newXPos);

            var _equilizeNoOfElements18 = slicedToArray(_equilizeNoOfElements17, 2);

            oldXPos = _equilizeNoOfElements18[0];
            newXPos = _equilizeNoOfElements18[1];

            var _equilizeNoOfElements19 = equilizeNoOfElements(oldYPos, newYPos);

            var _equilizeNoOfElements20 = slicedToArray(_equilizeNoOfElements19, 2);

            oldYPos = _equilizeNoOfElements20[0];
            newYPos = _equilizeNoOfElements20[1];

            var _equilizeNoOfElements21 = equilizeNoOfElements(oldOffsets, newOffsets);

            var _equilizeNoOfElements22 = slicedToArray(_equilizeNoOfElements21, 2);

            oldOffsets = _equilizeNoOfElements22[0];
            newOffsets = _equilizeNoOfElements22[1];

            var _equilizeNoOfElements23 = equilizeNoOfElements(oldLabels, newLabels);

            var _equilizeNoOfElements24 = slicedToArray(_equilizeNoOfElements23, 2);

            oldLabels = _equilizeNoOfElements24[0];
            newLabels = _equilizeNoOfElements24[1];
    ...
    lineGraph: {
        ...
        animateElements: function animateElements(newData) {
            ...
            var _equilizeNoOfElements25 = equilizeNoOfElements(oldXPos, newXPos);

            var _equilizeNoOfElements26 = slicedToArray(_equilizeNoOfElements25, 2);

            oldXPos = _equilizeNoOfElements26[0];
            newXPos = _equilizeNoOfElements26[1];

            var _equilizeNoOfElements27 = equilizeNoOfElements(oldYPos, newYPos);

            var _equilizeNoOfElements28 = slicedToArray(_equilizeNoOfElements27, 2);

            oldYPos = _equilizeNoOfElements28[0];
            newYPos = _equilizeNoOfElements28[1];

            var _equilizeNoOfElements29 = equilizeNoOfElements(oldValues, newValues);

            var _equilizeNoOfElements30 = slicedToArray(_equilizeNoOfElements29, 2);

            oldValues = _equilizeNoOfElements30[0];
            newValues = _equilizeNoOfElements30[1];
...
var PercentageChart = function (_AggregationChart) {
	...
	createClass(PercentageChart, [{
		...
		value: function calc() {
			...
			get(PercentageChart.prototype.__proto__ || Object.getPrototypeOf(PercentageChart.prototype), 'calc', this).call(this);
			var s = this.state;
...
function calcChartIntervals(values) {
	...
	// CASE II: Only minValue negative
			...
			if (maxValue >= absMinValue) {
				...
				intervals = posIntervals.reverse().map(function (d) {
					return d * -1;
				});
			}
		}
...
function getShortenedLabels(chartWidth) {
    ...
    var seriesMultiple = void 0;
    if (isSeries) {
        // Find the maximum label length for spacing calculations
        var maxLabelLength = Math.max.apply(Math, toConsumableArray(labels.map(function (label) {
            return label.length;
        })));
...
var AxisChart = function (_BaseChart) {
    ...
    createClass(AxisChart, [{
        ...
        value: function configure(options) {
            get(AxisChart.prototype.__proto__ || Object.getPrototypeOf(AxisChart.prototype), 'configure', this).call(this, options);
            var _options$axisOptions = options.axisOptions,
                axisOptions = _options$axisOptions === undefined ? {} : _options$axisOptions;

            var _ref = axisOptions || {},
                xAxis = _ref.xAxis,
                yAxis = _ref.yAxis;
            ...
            if (yAxis && yAxis.length) {
                this.config.yAxisConfig = yAxis.map(function (item) {
                    return {
        ...
        value: function calcYAxisParameters(dataValues) {
            ...
            var yPts = void 0,
                scaleMultiplier = void 0,
                intervalHeight = void 0,
                zeroLine = void 0,
                positions = void 0;
            ...
            if (dataValues instanceof Array) {
                ...
                var _loop = function _loop(key) {
                    var dataValue = dataValues[key];
                    yPts = calcChartIntervals(dataValue, withMinimum);
                    scaleMultiplier = _this2.height / getValueRange(yPts);
                    intervalHeight = getIntervalSize(yPts) * scaleMultiplier;
                    zeroLine = _this2.height - getZeroIndex(yPts) * intervalHeight;
                    positions = yPts.map(function (d) {
                        return zeroLine - d * scaleMultiplier;
                    });

                    var yAxisConfigObject = _this2.config.yAxisConfig.find(function (item) {
                        return key === item.id;
                    }) || [];
                    var yAxisAlignment = yAxisConfigObject ? yAxisConfigObject.position : 'right';

                    if (_this2.state.yAxis.length) {
                        var yPtsArray = [];
        ...
        value: function calcDatasetPoints() {
            var s = this.state;
            var scaleAll = function scaleAll(values, id) {
                return values.map(function (val) {
                    var yAxis = s.yAxis;


                    if (yAxis instanceof Array) {
                        yAxis = yAxis.find(function (axis) {
                            return id === axis.axisID;
                        });
                    }
            ...
            s.datasets = this.data.datasets.map(function (d, i) {
                ...
                return {
                    name: d.name && d.name.replace(/<|>|&/g, function (char) {
                        return char == '&' ? '&amp;' : char == '<' ? '&lt;' : '&gt;';
                    }),
                    index: i,
        ...
        value: function getAllYValues() {
            ...
            var key = 'values';
            var multiAxis = this.config.yAxisConfig ? true : false;
            var allValueLists = multiAxis ? {} : [];

            var groupBy = function groupBy(arr, property) {
                return arr.reduce(function (acc, cur) {
                    acc[cur[property]] = [].concat(toConsumableArray(acc[cur[property]] || []), [cur]);
                    return acc;
                }, {});
            };

            var generateCumulative = function generateCumulative(arr) {
                var cumulative = new Array(_this3.state.datasetLength).fill(0);
                arr.forEach(function (d, i) {
                    var values = arr[i].values;
            ...
            if (this.barOptions.stacked) {
                ...
                if (multiAxis) {
                    var groupedDataSets = groupBy(this.data.datasets, 'axisID');
            ...
            if (multiAxis) {
                this.data.datasets.forEach(function (d) {
                    var _allValueLists;

                    // if the array exists already just push more data into it.
                    // otherwise create a new array into the object.
                    allValueLists[d.axisID || key] ? (_allValueLists = allValueLists[d.axisID || key]).push.apply(_allValueLists, toConsumableArray(d[key])) : allValueLists[d.axisID || key] = [].concat(toConsumableArray(d[key]));
                });
            } else {
                allValueLists = this.data.datasets.map(function (d) {
                    return d[key];
                });
            }
            ...
            return multiAxis ? allValueLists : (_ref2 = []).concat.apply(_ref2, toConsumableArray(allValueLists));
        }
    }, {
        key: 'setupComponents',
        value: function setupComponents() {
            ...
            if (this.config.yAxisConfig && this.config.yAxisConfig.length) {
                this.config.yAxisConfig.forEach(function (yAxis) {
                    componentConfigs.push(['yAxis', {
                        mode: _this4.config.yAxisMode,
                        width: _this4.width,
                        shortenNumbers: _this4.config.shortenYAxisNumbers,
            ...
            var barsConfigs = barDatasets.map(function (d) {
                var index = d.index;
                var barIndex = d.barIndex || index;
                return ['barGraph' + '-' + d.index, {
                    index: index,
                    color: _this4.colors[index],
                    stacked: _this4.barOptions.stacked,

                    // same for all datasets
                    valuesOverPoints: _this4.config.valuesOverPoints,
                    minHeight: _this4.height * MIN_BAR_PERCENT_HEIGHT
                }, function () {
                    var s = this.state;
                    var yAxis = s.yAxis;
                    ...
                    var _d$id = d.id,
                        id = _d$id === undefined ? 'left-axis' : _d$id;
            ...
            var lineConfigs = lineDatasets.map(function (d) {
                ...
                return ['lineGraph' + '-' + d.index, {
                    index: index,
                    color: _this4.colors[index],
                    svgDefs: _this4.svgDefs,
                    heatline: _this4.lineOptions.heatline,
                    regionFill: _this4.lineOptions.regionFill,
                    spline: _this4.lineOptions.spline,
                    hideDots: _this4.lineOptions.hideDots,
                    hideLine: _this4.lineOptions.hideLine,
                    ...
                    var yAxis = s.yAxis.length ? s.yAxis.find(function (axis) {
                        return d.id === axis.axisID;
                    }) : s.yAxis;

                    var minLine = yAxis.positions[0] < yAxis.zeroLine ? yAxis.positions[0] : yAxis.zeroLine;
            ...
            this.components = new Map(componentConfigs.filter(function (args) {
                return !optionals.includes(args[0]) || _this4.state[args[0]];
            }).map(function (args) {
                var component = getComponent.apply(undefined, toConsumableArray(args));
                if (args[0]) {
                    if (args[0].includes('lineGraph') || args[0].includes('barGraph')) {
                        _this4.dataUnitComponents.push(component);
                    }
        ...
        value: function makeDataByIndex() {
            ...
            titles.map(function (label, index) {
                var values = _this5.state.datasets.map(function (set$$1, i) {
        ...
        value: function bindTooltip() {
            ...
            this.container.addEventListener('mousemove', function (e) {
                var m = _this6.measures;
                var o = getOffset(_this6.container);
                var relX = e.pageX - o.left - getLeftOffset(m);
                var relY = e.pageY - o.top;

                if (relY < _this6.height + getTopOffset(m) && relY > getTopOffset(m)) {
                    _this6.mapTooltipXPosition(relX);
                } else {
                    _this6.tip.hideTip();
                }
            });
        ...
        value: function renderLegend() {
            ...
            if (s.datasets.length > 1) {
                ...
                s.datasets.map(function (d, i) {
                    ...
                    barWidth * i, '0', barWidth, _this7.colors[i], d.name, _this7.config.truncateLegends);
                    _this7.legendArea.appendChild(rect);
                });
            }
        }
        ...
        value: function makeOverlay$$1() {
            ...
            this.overlayGuides.map(function (d) {
                var currentUnit = d.units[_this8.state.currentIndex];

                d.overlay = makeOverlay[d.type](currentUnit);
                _this8.drawArea.appendChild(d.overlay);
            });
        }
    }, {
        ...
        value: function bindUnits() {
            ...
            this.dataUnitComponents.map(function (c) {
                c.units.map(function (unit) {
                    unit.addEventListener('click', function () {
                        ...
                        _this10.setCurrentDataPoint(index);
                    });
                });
            });
            ...
            this.tip.container.addEventListener('click', function () {
                var index = _this10.tip.container.getAttribute('data-point-index');
                _this10.setCurrentDataPoint(index);
            });
        }
    }, {
        ...
        value: function updateOverlay$$1() {
            ...
            this.overlayGuides.map(function (d) {
                var currentUnit = d.units[_this11.state.currentIndex];
...
var DonutChart = function (_AggregationChart) {
	inherits(DonutChart, _AggregationChart);

	function DonutChart(parent, args) {
		classCallCheck(this, DonutChart);

		var _this = possibleConstructorReturn(this, (DonutChart.__proto__ || Object.getPrototypeOf(DonutChart)).call(this, parent, args));
	...
	createClass(DonutChart, [{
		key: 'configure',
		value: function configure(args) {
			get(DonutChart.prototype.__proto__ || Object.getPrototypeOf(DonutChart.prototype), 'configure', this).call(this, args);
			this.mouseMove = this.mouseMove.bind(this);
			this.mouseLeave = this.mouseLeave.bind(this);
		...
		value: function calc() {
			...
			get(DonutChart.prototype.__proto__ || Object.getPrototypeOf(DonutChart.prototype), 'calc', this).call(this);
			var s = this.state;
